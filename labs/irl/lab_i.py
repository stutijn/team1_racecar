...
def adjust_angle(kp, d_right, d_left):
    setpoint = (abs(d_right) + abs(d_left)) / 2
    speed = 0.5
...
def update_new():
    global speed, angle
    scan = rc.lidar.get_samples()
    a_r, right = rc_utils.get_lidar_closest_point(scan, [45, 135])
    a_l, left = rc_utils.get_lidar_closest_point(scan, [225, 315])
    right_forward = rc_utils.get_lidar_average_distance(scan, 15, 60)
    left_forward = rc_utils.get_lidar_average_distance(scan, 300, 345)
    right_wall = rc_utils.get_lidar_average_distance(scan, 90, 10)
    left_wall = rc_utils.get_lidar_average_distance(scan, 270, 10)
    front_window = rc_utils.get_lidar_average_distance(scan, 340, 20)
    ...
    if right_forward - left_forward < 5:
        angle = 0
        speed = 0.7
    elif right_forward - left_forward > 5:
        angle, speed = adjust_angle(kp, right_forward, left_forward)
    ...
    if speed < 0.5:
        speed = 0.7
    print(right_forward, left_forward)
