...
COLOR_PRIORITY = (ORANGE, PURPLE, RED, GREEN, BLUE)
...
def update_contour():
    ...
    global contour_area
    global w_contour_area
    global w_contour_center

    image = rc.camera.get_color_image()
    t_image = rc.camera.get_color_image()
    if image is None:
        contour_center = None
        contour_area = 0
    else:
        ...
        for color in COLOR_PRIORITY:
            ...
            lrg_contour = rc_utils.get_largest_contour(contours, MIN_CONTOUR_AREA)
            if lrg_contour is not None:
                contour_center = rc_utils.get_contour_center(lrg_contour)
                contour_area = rc_utils.get_contour_area(lrg_contour)
    ...
    #     w_contour_area = 0
    # else:
    #     if rc_utils.find_contours(t_image, ORANGE[0], ORANGE[1]) is not None or rc_utils.find_contours(t_image, PURPLE[0], PURPLE[1]) is not None:
    #         t_image = rc_utils.crop(t_image, TINY_CROP[0], TINY_CROP[1])
    #         contours_o = rc_utils.find_contours(t_image, ORANGE[0], ORANGE[1])
    #         contours_p = rc_utils.find_contours(t_image, PURPLE[0], PURPLE[1])
    ...
    #             w_contour_center = rc_utils.get_contour_center(lrg_contour_o)
    #             w_contour_area = rc_utils.get_contour_area(lrg_contour_o)
    #         elif lrg_contour_p is not None:
    #             w_contour_center = rc_utils.get_contour_center(lrg_contour_p)
    #             w_contour_center = rc_utils.get_contour_center(lrg_contour_p)
    #         else:
    #             pass
...
def update():
    ...
    if contour_center is not None:
        if contour_center == PURPLE or contour_center == ORANGE:
            setpoint = rc.camera.get_width() // 2
            present_value = contour_center[1]
            kp = 0.00325
            error = present_value - setpoint # setpoint - present_value
