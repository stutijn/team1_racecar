...
current_state = ""
right_commands = [[1, 0.5, .5],  [0.25, 0.25, 0], [2, 0, 0], [1, 0.5, -1]] 
                #   [0.1, 0.75, -1], [0.1, 0.75, -1], [0.1, 0.75, -1], 
                #   [0.1, 0.75, -1], [0.5, 0.5, 0], [0.5, 0, 0]]

left_commands = [[0.125, 0.6, -1], [.25, .25, 1], [2, 0, 0],  [0.25, 0.75, 1]] 
                #  [0.1, 0.75, 1], [0.1, 0.75, 1], [0.1, 0.75, 1], 
                #  [0.1, 0.75, 1], [0.5, 0.5, 0], [0.5, 0, 0]]
...
def update_contour():
    ...
    if image is None:
        ...
        # print(f"Blue contour: {lrg_contour_b}, Red contour: {lrg_contour_r}")
        ...
        # print(f"Blue area: {contour_b_area}, Red area: {contour_r_area}")
        # print(f"White contour: {lrg_contour_w}")
...
def navigate_red():
    global speed, angle
    queue.clear()
    # queue.extend(right_commands)

    # queue.append([0.5, 0.35, .55])
    # # if contour_r_area == 0:
    # #     current_state = STATES[2]
    # # queue.append([0.65, 0.75, -1])
    # queue.append([0.25, 0.25, 0])
...
def update_new():
    ...
    if len(queue) > 0:
        ...
        if queue[0][0] <= 0:
            queue.pop(0)
    # kp = 0.003125
    # x_dist = distance_c * math.sin(angle_c)
    # if distance_c > 125 and x_dist > 0:
    #     speed = 0.75
    #     # angle = (clamp_cone_angle(angle_c) % 90) / 90
    #     setpoint = 0
    #     error = setpoint - x_dist
    #     angle = kp * error
    # elif distance_c <= 125:
    #     angle = 0
    else:
        ...
        if turn_right(contour_r_area, contour_b_area):
            ...
            angle_c, distance_c = rc_utils.get_lidar_closest_point(scan, (315, 45))
            angle = 0 #(clamp_cone_angle(angle_c) % 90) / 90
            speed = .5
    # print(f"Wheel angle: {angle}; Distance: {distance_c}; Angle to cone: {angle_c}")
...
def update_slow():
    # if right < 75 or left < 75:
    #     print("Thingy in front! 🤯")
    pass
