...
def red_cone():
    global current_state
    queue.clear()
    queue.append([2, 0.75, 0.5])
    queue.append([1.5, 0.5, 0])
...
def update():
    ...
    angle_c, distance_c = rc_utils.get_lidar_closest_point(scan, (290, 70))
    # if angle_c > 355 or angle_c < 5:
    #     angle = 0
    # elif angle_c < 70: 
    #     angle = .5
    # elif angle_c > 290:
    #     angle = -.5
    print(f"Angle_l: {angle_l}; Distance_l: {distance_l}; Angle_r: {angle_r}; Distance_r: {distance_r}")
    contour_r_area = 0
    contour_b_area = 0
